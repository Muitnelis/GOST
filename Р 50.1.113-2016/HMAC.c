...
unsigned char* HMAC(unsigned char* K_file_path, unsigned char* T_file_path, unsigned char* output_file_path, int bits)
{
    int bytes = bits >> 3;
    ...
    streebog("streebog_input.txt", "streebog_output.txt", bits);
    ...
    buffer_len = fread(buffer, sizeof(unsigned char), bytes, streebog_output_1);
    fwrite(buffer, sizeof(unsigned char), bytes, streebog_input_2);
    ...
    memcpy(buffer, streebog("streebog_input.txt", "streebog_output.txt", bits), bytes);
    ...
    if (output_file_path)
    {
        ...
        fwrite(result, sizeof(unsigned char), bytes, HMAC_result);
        fclose(HMAC_result);
    }
